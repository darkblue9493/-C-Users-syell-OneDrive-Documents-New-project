...
function starterDatabase() {
  return { chats: starterChats, users: [], pointTransactions: [] };
}
...
function normalizeDatabase(data) {
  ...
  normalized.chats = normalized.chats.filter((chat) => !["demo-maya", "demo-andre"].includes(chat.id));
  return normalized;
}
...
async function handleApi(request, response, urlPath, url) {
  ...
  if (request.method === "POST" && urlPath === "/api/admin/points") {
    ...
    if (action === "redeem" && amount < 20) {
      return sendJson(response, 400, { error: "Minimum redemption is 20 points." });
    }
