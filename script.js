...
const chatNav = document.querySelector("[data-chat-nav]");
...
const adminChatJump = document.querySelector("[data-admin-chat-jump]");
const adminUnreadBadge = document.querySelector("[data-chat-unread-count]");
...
adminPanelButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    adminChatJump?.classList.remove("is-active");
...
if (adminChatJump) {
  adminChatJump.addEventListener("click", () => {
    adminPanelButtons.forEach((item) => item.classList.remove("is-active"));
    adminChatJump.classList.add("is-active");
...
function renderUsers(users) {
  ...
  if (!visibleUsers.length) {
    adminUsers.innerHTML = `<article class="registered-player"><strong>No matching players.</strong><span></span><small></small></article>`;
    return;
  }
  ...
  visibleUsers.forEach((user) => {
    ...
    item.innerHTML = `
      <div class="registered-player-identity">
        ...
        <button class="registered-player-name" type="button" data-open-user-chat></button>
        <span class="registered-player-email"></span>
      </div>
      <span></span>
      <small></small>
      <small></small>
      ...
      <form class="admin-note-form" data-note-user>
        <label>Private note</label>
        <textarea name="note" rows="2" placeholder="VIP, bonus requests, payment notes..."></textarea>
        <button class="game-link" type="submit">Save Note</button>
      </form>
    `;
    item.querySelector("[data-reset-user]").dataset.userId = user.id;
    item.querySelector("[data-note-user]").dataset.userId = user.id;
    setPresenceDot(item.querySelector(".player-presence-dot"), presence);
    item.querySelector("[data-open-user-chat]").dataset.userId = user.id;
    item.querySelector("[data-open-user-chat]").textContent = user.username;
    item.querySelector(".registered-player-email").textContent = `${presence.label} | ${user.email}`;
    ...
    item.querySelector("[name='note']").value = user.adminNote || "";
...
async function renderAdmin() {
  ...
  if (adminUnreadBadge) {
    adminUnreadBadge.textContent = unreadTotal;
    adminUnreadBadge.classList.toggle("is-hidden", unreadTotal <= 0);
  }
  ...
  chats.forEach((chat) => {
    ...
    button.innerHTML = `<span class="inbox-player-name"><i class="player-presence-dot"></i><b></b><em class="inbox-unread is-hidden"></em></span><small></small>`;
