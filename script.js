...
function renderPointsActionLists(users) {
  if (pointsAddList) {
    pointsAddList.innerHTML = users.length ? users.map((user) => pointsActionCard(user, "add")).join("") : `<article class="points-action-card">No registered players yet.</article>`;
    users.forEach((user, index) => {
      pointsAddList.querySelectorAll("[data-points-user]")[index].dataset.userId = user.id;
    });
  }
  if (pointsRedeemList) {
    pointsRedeemList.innerHTML = users.length ? users.map((user) => pointsActionCard(user, "redeem")).join("") : `<article class="points-action-card">No registered players yet.</article>`;
    users.forEach((user, index) => {
