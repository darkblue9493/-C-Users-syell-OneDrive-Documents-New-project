...
if (signupDateOfBirth) {
  ...
  signupDateOfBirth.max = maxDate.toISOString().slice(0, 10);
}
...
async function renderAdmin() {
  ...
  const isResetFormActive = Boolean(adminUsers?.contains(document.activeElement) && document.activeElement.closest("[data-reset-user]"));
  if (!isResetFormActive && usersKey !== lastAdminUsersKey) {
    renderUsers(users);
