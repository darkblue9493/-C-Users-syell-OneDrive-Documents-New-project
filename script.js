...
function showPlayerAuth() {
  ...
  playerAppSections.forEach((section) => section.classList.add("is-hidden"));
}

authTabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    authTabs.forEach((item) => item.classList.toggle("is-active", item === tab));
    authPanels.forEach((panel) => panel.classList.toggle("is-active", panel.dataset.playerSignup !== undefined && tab.dataset.authTab === "signup"));
...
function renderUsers(users) {
  ...
  users.forEach((user) => {
    ...
    item.innerHTML = `<div><strong></strong><span></span></div><span></span><small></small>`;
